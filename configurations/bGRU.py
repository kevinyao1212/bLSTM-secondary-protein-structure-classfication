...
weights = {
    ...
    'h2': tf.Variable(tf.random_normal([N_GRU_F*2, N_L2])),
...
def build_model(x, weights, biases):
    ...
    GRU_backward = rnn.BasicLSTMCell(N_GRU_B)

    #state = tf.zeros([batch_size, lstm_forward.state_size])

    #lstm_output, state = lstm_forward(layer_1, state)
    GRU_output, _, _ = rnn.static_bidirectional_rnn(GRU_forward, GRU_backward, layer_1, dtype=tf.float32)

    GRU_output = tf.reshape(GRU_output,[batch_size*seq_len,2*N_GRU_F])
